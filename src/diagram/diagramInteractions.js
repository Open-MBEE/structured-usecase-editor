...
const MIN_USE_CASE_WIDTH = 110;
const MIN_USE_CASE_HEIGHT = 56;
...
export function installDiagramDragHandlers() {
  ...
  svg.querySelectorAll("[data-resize-handle]").forEach((element) => {
    element.addEventListener("mousedown", (event) => {
      const node = findNode(element.dataset.nodeId);
      if (!node || nodeKind(node) !== "usecase") return;
      const layout = nodeLayout(node);
      const point = diagramPointFromEvent(svg, event);
      selectNode(node.id);
      state.useCaseResize = {
        nodeId: node.id,
        handle: element.dataset.resizeHandle,
        startX: point.x,
        startY: point.y,
        layout: { ...layout }
      };
  ...
  if (!window.__structuredUseCaseDiagramDragInstalled) {
    ...
    window.addEventListener("mousemove", (event) => {
      if (state.connectionDrag) {
        ...
        state.connectionDrag.currentX = Math.max(DIAGRAM_MARGIN, Math.min(DIAGRAM_WIDTH - DIAGRAM_MARGIN, point.x));
        state.connectionDrag.currentY = Math.max(DIAGRAM_MARGIN, Math.min(DIAGRAM_HEIGHT - DIAGRAM_MARGIN, point.y));
      ...
      if (state.useCaseResize) {
        const activeSvg = document.getElementById("useCaseDiagramCanvas");
        if (!activeSvg) return;
        const node = findNode(state.useCaseResize.nodeId);
        if (!node) return;
        const point = diagramPointFromEvent(activeSvg, event);
        const dx = point.x - state.useCaseResize.startX;
        const dy = point.y - state.useCaseResize.startY;
        const layout = nodeLayout(node);
        const original = state.useCaseResize.layout;
        const handle = state.useCaseResize.handle;
        ...
        if (handle.includes("e")) nextWidth = original.width + dx;
        if (handle.includes("s")) nextHeight = original.height + dy;
        ...
        if (nextWidth < MIN_USE_CASE_WIDTH) {
          if (handle.includes("w")) nextX -= MIN_USE_CASE_WIDTH - nextWidth;
          nextWidth = MIN_USE_CASE_WIDTH;
        }
        if (nextHeight < MIN_USE_CASE_HEIGHT) {
          if (handle.includes("n")) nextY -= MIN_USE_CASE_HEIGHT - nextHeight;
          nextHeight = MIN_USE_CASE_HEIGHT;
        }

        nextX = Math.max(DIAGRAM_MARGIN, Math.min(DIAGRAM_WIDTH - nextWidth - DIAGRAM_MARGIN, nextX));
        nextY = Math.max(DIAGRAM_MARGIN, Math.min(DIAGRAM_HEIGHT - nextHeight - DIAGRAM_MARGIN, nextY));
        layout.x = nextX;
        layout.y = nextY;
        layout.width = Math.min(nextWidth, DIAGRAM_WIDTH - layout.x - DIAGRAM_MARGIN);
        layout.height = Math.min(nextHeight, DIAGRAM_HEIGHT - layout.y - DIAGRAM_MARGIN);
        render();
        return;
      }
    ...
    window.addEventListener("mouseup", (event) => {
      ...
      if (state.useCaseResize) saveProjectSnapshot();
      state.useCaseResize = null;
